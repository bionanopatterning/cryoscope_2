...
def clb_ThicknessImageMacro():
    ...
    if not channel_R:
        ...
        prm.liveChannels[-1].leds = [False, False, False, False, False, False, True]
    ...
    if not channel_G:
        ...
        prm.liveChannels[-1].filterCube = 1
        prm.liveChannels[-1].leds = [False, False, False, False, False, True, False]
        prm.liveChannels[-1].exposureTime = 100
    ...
    if not channel_B:
        ...
        prm.liveChannels[-1].filterCube = 1
        prm.liveChannels[-1].leds = [False, False, False, False, True, False, False]
        prm.liveChannels[-1].exposureTime = 100
...
def AcquisitionGUI():
    ...
    def _overallGUI():
        imgui.text("ACQUISITION MODE SOURCE POWERS")
        i = 0
        imgui.new_line()
        for source in prm.lightSources:
            ...
            _, prm.acquisitionPower[i] = imgui.slider_float(source, prm.acquisitionPower[i], 0.0, 100.0, format = "%.0f")
            imgui.pop_style_color(3)
...
def SearchGUI():
    ...
    def _overallGUI():
        imgui.text("SEARCH MODE SOURCE POWERS")
        i = 0
        imgui.new_line()
        ...
        for source in prm.lightSources:
            ...
            imgui.push_style_color(imgui.COLOR_FRAME_BACKGROUND, cfg.sliderBackgroundColor[0], cfg.sliderBackgroundColor[1], cfg.sliderBackgroundColor[2])
            imgui.push_style_color(imgui.COLOR_SLIDER_GRAB, wavelengthColour[0], wavelengthColour[1], wavelengthColour[2])
            _change, prm.livePower[i] = imgui.slider_float(source, prm.livePower[i], 0.0, 100.0, format = "%.0f")
...
def ChannelGUI_Simple(channel, allowCondenser = False):
    ...
    for source in prm.lightSources:
        if i % 4 == 0:
            ...
            imgui.same_line((i % 4) * cfg.source_checkbox_spacing)
